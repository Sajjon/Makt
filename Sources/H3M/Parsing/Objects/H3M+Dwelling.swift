...
internal extension H3M {
    ...
    func parseDwelling(attributes: Map.Object.Attributes) throws -> Map.Dwelling {
        ...
        func parsePossibleFactions() throws -> Factions {
            try .init(values: reader.readUInt32() != 0 ? .allCases : parseBitmaskOfEnum(byteCount: 2))
        }
        ...
        case .randomDwellingOfFaction(let faction):
            let range = try getLevelRange()
            return .random(.init(
                possibleFactions: .init(values: [faction]),
                possibleLevels: .range(range)
            ), owner: owner)
        ...
        default: incorrectImplementation(shouldAlreadyHave: "Handled all cases of dwelling, object attrubutes: \(attributes)")
        }
