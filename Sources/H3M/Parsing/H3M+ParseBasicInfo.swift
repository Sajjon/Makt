...
extension  H3M {
    ...
    func parseBasicInfo(
        ...
        let name = try reader.readLengthOfStringAndString(assertingMaxLength: 30) ?? "" // Cyon verified that 30 chars is max for "Name" in Map Editor
        inspector?.didParseName(name)
        
        let description = try reader.readLengthOfStringAndString(assertingMaxLength: 300) ?? ""  // Cyon verified that 300 chars is max for "Description" in Map Editor
